...
for image in images:
    ...
    # aplicamos a rotação de 45 graus
    t_image = cv2.getRotationMatrix2D((cols / 2, rows / 2), 45, 1)
    dst = cv2.warpAffine(original_img, t_image, (cols, rows))
    image_handler.add_img_to_plot(dst, 'Translated Image', 1, 4, 3)

    # aplicamos a transformada na imagem rotacionada
    t_magnitude_spectrum, fft_shift = image_handler.apply_fourier(dst)
